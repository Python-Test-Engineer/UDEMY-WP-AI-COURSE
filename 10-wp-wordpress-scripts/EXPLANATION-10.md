...
function my_plugin_enqueue_scripts($hook) {
    ...
    // The filenames and locations can be changed. These are the default ones used by @wordpress/scripts.
    ...
    // wp_localize_script is a WordPress function that passes data from PHP to JavaScript. It creates a JavaScript object that your scripts can access.
    // Basic syntax:
    // phpwp_localize_script( 'script-handle', 'objectName', $data_array );

    // Parameters:
    // script-handle - 'my-custom-app' - The registered script you want to pass data to
    // objectName - 'myPluginData' - The JavaScript variable name (will be globally accessible)
    // $data_array - PHP array of data to pass
    // Accessible in JS as ${window.myPluginData.apiUrl}
