 ...
 * Description: AI-powered search assistant using Full Text Search and Vector Search APIs
 * Version: 1.0.0
 * Author: Your Name
...
class RAG_Search_Assistant {
    ...
    private function generate_answer($query, $context) {
        ...
        $api_key = get_option('rag_anthropic_api_key', '');
        
        if (empty($api_key)) {
            return "Please configure your Anthropic API key in the plugin settings.";
        }
        ...
        $api_response = wp_remote_post('https://api.anthropic.com/v1/messages', array(
            ...
            'body' => json_encode(array(
                ...
                'messages' => array(
                    array(
                        'role' => 'user',
                        'content' => "Based on the following context from our product database, please answer the user's question. If the context doesn't contain enough information to answer the question, respond with 'My RAG does not have the answer.'\n\nContext:\n{$context}\n\nQuestion: {$query}\n\nAnswer:"
                    )
                )
            )),
            'timeout' => 30
        ...
        if (is_wp_error($api_response)) {
            error_log('Claude API Error: ' . $api_response->get_error_message());
        ...
        if (isset($response_body['error'])) {
            error_log('Claude API Error: ' . print_r($response_body['error'], true));
...
add_action('admin_init', 'rag_search_register_settings');
function rag_search_register_settings() {
    register_setting('rag_search_options', 'rag_anthropic_api_key');
}

add_action('admin_menu', 'rag_search_add_settings_page');
function rag_search_add_settings_page() {
    add_submenu_page(
        'rag-search-assistant',
        'RAG Settings',
        'Settings',
        'manage_options',
        'rag-search-settings',
        'rag_search_settings_page'
    );
}

function rag_search_settings_page() {
    ?>
    <div class="wrap">
        <h1>RAG Search Assistant Settings</h1>
        <form method="post" action="options.php">
            <?php
            settings_fields('rag_search_options');
            do_settings_sections('rag_search_options');
            ?>
            <table class="form-table">
                <tr>
                    <th scope="row">
                        <label for="rag_anthropic_api_key">Anthropic API Key</label>
                    </th>
                    <td>
                        <input type="password" id="rag_anthropic_api_key" name="rag_anthropic_api_key" value="<?php echo esc_attr(get_option('rag_anthropic_api_key', '')); ?>" class="regular-text">
                        <p class="description">Enter your Anthropic API key to enable AI-powered answers.</p>
                    </td>
                </tr>
            </table>
            <?php submit_button(); ?>
